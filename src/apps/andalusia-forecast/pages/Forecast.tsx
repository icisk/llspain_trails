...
export function Forecast() {
    ...
    const precipitationService = useService<PrecipitationLayerHandler>(
        "app.PrecipitationLayerHandler"
    );
    const prepSrvc = useService<PrecipitationLayerHandler>("app.PrecipitationLayerHandler");
    const [curVar, curForecast, curForecastMonths, showUncert] = useReactiveSnapshot(
        () => [
    ...
    const variables = [{ temp: "Temperatura" }, { precip: "Precipitación" }];
    ...
    useEffect(() => {
        ...
        if (curVar === "temp") {
            newOpts = {
                title: { text: intl.formatMessage({ id: "global.plot.header_temp" }) },
                ...
                <HStack flex={"overflow"}>
                    <HStack>
                        <Box whiteSpace={"nowrap"}>
                            {intl.formatMessage({ id: "global.controls.sel_prediction" })}{" "}
                        </Box>
                        <Select>
