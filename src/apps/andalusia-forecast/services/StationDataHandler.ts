...
export class StationDataHandlerImpl implements StationDataHandler {
    ...
    async fetchStationsData(): Promise<any> {
        try {
            const response = await fetch(
                "https://i-cisk.dev.52north.org/data/collections/ll_spain_aemet_stations/items?f=json&limit=1000"
            );
            if (!response.ok) {
    ...
    setStations() {
        this.#allStations = this.#data.features
            .map((e) => ({
                NAME_STATION: e.properties.NAME_STATION,
                CODE_INM: e.properties.CODE_INM
            }))
            .sort((a, b) => {
